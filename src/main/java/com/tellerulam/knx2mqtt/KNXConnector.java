...
public class KNXConnector extends Thread implements NetworkLinkListener {
		...
		if (connType != null) {
			if ("TUNNELING".equals(connType))
				...
				throw new IllegalArgumentException(
						"knx2mqtt.knx.type must bei either TUNNELING, ROUTING or SERIAL");
		}
	...
	private void connectIP(int knxConnectionType)
			throws KNXException, InterruptedException {
		String hostIP = System.getProperty(PropertyNames.KNX2MQTT_KNX_IP,
				"setme");
		int port = Integer.getInteger(PropertyNames.KNX2MQTT_KNX_PORT,
				KNXnetIPConnection.DEFAULT_PORT).intValue();
		...
		if (localIP != null) {
			...
			try {
				...
				throw new IllegalArgumentException(
						"Unable to determine local host address");
			}
			local = new InetSocketAddress(localhost, 0);
		}
		L.log(Level.INFO,
				"Establishing KNX IP connection to " + hostIP + ":" + port
						+ " ("
						+ (knxConnectionType == KNXNetworkLinkIP.TUNNELING
								? "TUNNEL"
								: "ROUTER")
						+ ") from " + local);
		link = new KNXNetworkLinkIP(knxConnectionType, local,
				new InetSocketAddress(hostIP, port), false, TPSettings.TP1);
		L.info("KNX IP Connection established");
	}

	private void connectSerial() {
		throw new IllegalArgumentException(
				"Serial connection not yet implemented");
	}
	...
	private class MyProcessListener extends ProcessListenerEx {
		...
		public void groupWrite(ProcessEvent pe) {
			...
			GroupAddressInfo gaInfo = GroupAddressManager
					.getGAInfoForAddress(dest.toString());
			...
			try {
				...
				if (gaInfo == null) {
					String dpt;
					if (asdu.length == 1) {
						val = Integer.valueOf(asUnsigned(pe,
					...
					L.info("Got " + val + " to unknown " + dest + " from " + src
							+ " (ASDU length " + asdu.length + ")");
					MQTTHandler.publish(dest.toString(), val, src.toString(),
							dpt, null, now, now);
				} else {
					MQTTHandler.publish(gaInfo.name,
							gaInfo.translateAndStoreValue(asdu, now),
							src.toString(), gaInfo.dpt, gaInfo.getTextutal(),
							now, gaInfo.lastValueTimestamp);
				}
			} catch (KNXException e) {
				L.log(Level.WARNING,
						"Error converting ASDU to " + dest + " from " + src);
			}
	...
	public void run() {
		for (;;) {
			try {
				...
				L.log(Level.WARNING,
						"Error in KNX connection, will retry in 10s", e);
				try {
					Thread.sleep(5 * 1000);
					if (pc != null)
						pc.detach();
	...
	private static byte[] createGroupAPDU(final int service,
			final DPTXlator t) {
		...
		final byte[] buf = new byte[t.getItems() * Math.max(1, t.getTypeSize())
				+ offset];
	...
	public static void doGroupWrite(String gaspec, String val,
			GroupAddressInfo gai) {
		try {
			...
			if (gai.xlator instanceof DPTXlatorBoolean) {
				...
				gai.xlator.setValue(val);
			conn.link.sendRequestWait(ga, Priority.LOW,
					createGroupAPDU(GROUP_WRITE, gai.xlator));
		} catch (Exception e) {
			conn.L.log(Level.WARNING,
					"Error when writing " + val + " to " + gaspec, e);
		}
	}

	public static void doGroupRead(String gaspec, String val,
			GroupAddressInfo gai) {
		try {
			GroupAddress ga = new GroupAddress(gaspec);
			conn.link.sendRequestWait(ga, Priority.LOW, DataUnitBuilder
					.createLengthOptimizedAPDU(GROUP_READ, null));
