...
public class GroupAddressManager {
	private static final Logger L = Logger
			.getLogger(GroupAddressManager.class.getName());
	...
	public static class GroupAddressInfo implements Serializable {
		...
		void createTranslator() throws KNXException {
			try {
				...
				L.warning("WARNING! Unable to create translator for DPT " + dpt
						+ " of " + name
	...
	static void loadGroupAddressTable() {
		String gaFile = System
				.getProperty(PropertyNames.KNX2MQTT_KNX_GROUPADDRESSTABLE);
		if (gaFile == null) {
		...
		try {
			...
			L.info("Read " + gaTable.size() + " Group Address entries from "
					+ gaFile);
		} catch (Exception e) {
			L.log(Level.SEVERE,
					"Unable to parse Group Address table file " + gaFile, e);
			System.exit(1);
		}
	}
	...
	static void loadETS4Project() {
		String gaFile = System
				.getProperty(PropertyNames.KNX2MQTT_KNX_ETS5PROJECTFILE);
		if (gaFile == null)
			gaFile = System
					.getProperty(PropertyNames.KNX2MQTT_KNX_ETS4PROJECTFILE);
		if (gaFile == null) {
		...
		if (cacheFile.exists()) {
			if (cacheFile.lastModified() > projectFile.lastModified()) {
				try (ObjectInputStream ois = new ObjectInputStream(
					...
					L.log(Level.WARNING, "Error reading cache file " + cacheFile
							+ ", ignoring it", e);
				}
			} else {
				L.info("Cache file " + cacheFile
						+ " exists, but project file is newer, ignoring it");
			}
		}
	...
	private static void storeGAInfo(String address, String name,
	...
	private static void processETS4ProjectFile(ZipFile zf, ZipEntry zep)
		...
		for (int ix = 0; ix < gas.getLength(); ix++) {
			...
			processETS4GroupAddressConnections(zf, doc, sendConnections,
					receiveConnections, e.getAttribute("Id"), address, name);
		}
	}
	 ...
	 */
	private static void processETS4GroupAddressConnections(ZipFile zf,
			Document doc, NodeList sendConnections, NodeList receiveConnections,
		...
		for (int attempt = 0; attempt < 4; attempt++) {
			...
			if (attempt == 2 && !foundConnection)
				break;
			NodeList connectors = ((attempt & 1) == 0) ? receiveConnections
					: sendConnections;
			...
			for (int ix = 0; ix < connectors.getLength(); ix++) {
				...
				if (id.equals(e.getAttribute("GroupAddressRefId"))) {
					...
					if (processETS4GroupConnection(zf, pe.getAttribute("RefId"),
							id, address, name, useObjectSize))
						return;
				}
			}
		}
		if (!foundConnection)
			...
			throw new IllegalArgumentException(
					"Unable to determine datapoint type for " + id + "/"
							+ address + "/" + name);
	}
	...
	private static Map<String, Map<String, String>> loadDeviceDescription(
			ZipFile zf, String filename)
			throws ParserConfigurationException, SAXException, IOException {
		if (deviceDescriptionCache == null) {
			saxFactory = SAXParserFactory.newInstance();
			deviceDescriptionCache = new HashMap<>();
		} else {
			Map<String, Map<String, String>> cacheEntry = deviceDescriptionCache
					.get(filename);
			if (cacheEntry != null) {
				return cacheEntry;
			}
		...
		if (ze == null)
			throw new IllegalArgumentException(
					"Unable to find device description " + filename);
	...
	private static boolean processETS4GroupConnection(ZipFile zf, String refId,
			String id, String address, String name, boolean useObjectSize)
			throws SAXException, IOException, ParserConfigurationException {
		// Right, we need to look into the device description. Determine it's
		// filename
		String refIdParts[] = refId.split("_");
		String pathName = refIdParts[0] + "/" + refIdParts[0] + "_"
				+ refIdParts[1] + ".xml";
		Map<String, Map<String, String>> dev = loadDeviceDescription(zf,
				pathName);
		Map<String, String> cobjref = dev.get(refId);
		if (cobjref == null)
			throw new IllegalArgumentException(
					"Unable to find ComObjectRef with Id " + refId + " in "
							+ pathName);
		...
		if (cobj == null)
			throw new IllegalArgumentException(
					"Unable to find ComObject with Id " + refco + " in "
							+ pathName);
	...
	private static boolean processETS4ComObj(Map<String, String> cobj,
			ZipFile zf, String address, String name, boolean useObjectSize)
			throws SAXException, IOException, ParserConfigurationException {
		...
		if (useObjectSize) {
			...
			if (objSize != null && objSize.length() != 0) {
				...
				if (!"1 Bit".equals(objSize))
					L.warning("Warning: Infering DPT for "
							+ new GroupAddress(Integer.parseInt(address)) + " ("
	...
	private static String inferDPTFromObjectSize(ZipFile zf, String objSize)
		...
		if ("1 Bit".equals(objSize))
			...
			if (dptMap == null) {
				...
				DefaultHandler gaHandler = new DefaultHandler() {
					...
					public void startElement(String uri, String localName,
						...
						if ("DatapointType".equals(qName)) {
							...
							if (currentSize != null) {
								dptMap.put(Integer.valueOf(currentSize),
										attr.getValue("Id"));
								currentSize = null;
							}
						}
				...
				saxParser.parse(
						zf.getInputStream(new ZipEntry("knx_master.xml")),
						gaHandler);
			}
