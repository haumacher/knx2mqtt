...
public class MQTTHandler {
	...
	private MQTTHandler() {
		String tp = System.getProperty(PropertyNames.KNX2MQTT_MQTT_TOPIC,
				"knx");
		if (!tp.endsWith("/"))
			tp += "/";
		topicPrefix = tp;
	...
	private void processSetGet(String namePart, MqttMessage msg, boolean set) {
		...
		if (gai == null) {
			L.warning("Unable to translate name " + namePart
					+ " into a group address, ignoring message " + msg);
			return;
		}
	...
	private void doConnect() {
		L.info("Connecting to MQTT broker " + mqttc.getServerURI()
				+ " with CLIENTID=" + mqttc.getClientId() + " and TOPIC PREFIX="
				+ topicPrefix);
		...
		try {
			...
			try {
				...
				L.log(Level.WARNING,
						"Error subscribing to topic hierarchy, check your configuration",
						mqe);
				throw mqe;
			}
		} catch (MqttException mqe) {
			L.log(Level.WARNING,
					"Error while connecting to MQTT broker, will retry: "
							+ mqe.getMessage(),
					mqe);
	...
	private void doInit() throws MqttException {
		String server = System.getProperty(PropertyNames.KNX2MQTT_MQTT_SERVER,
				"tcp://localhost:1883");
		String clientID = System
				.getProperty(PropertyNames.KNX2MQTT_MQTT_CLIENTID, "knx2mqtt");
		...
		mqttc.setCallback(new MqttCallback() {
			...
			public void messageArrived(String topic, MqttMessage msg)
				...
				try {
					...
					L.log(Level.WARNING, "Error when processing message " + msg
							+ " for " + topic, e);
				}
			}
	...
	private void doPublish(String name, Object val, String src, String dpt,
			String textual, long updateTime, long lastChange) {
		JsonObject jso = new JsonObject();
		jso.add("ts", updateTime).add("lc", lastChange).add("knx_src_addr", src)
				.add("knx_dpt", dpt);
		if (textual != null)
		...
		MqttMessage msg = new MqttMessage(
				jso.toString().getBytes(StandardCharsets.UTF_8));
		msg.setQos(0);
	...
	private void sendConnectionState() {
		try {
			instance.mqttc.publish(instance.topicPrefix + "connected",
					(knxConnectionState ? "2" : "1").getBytes(), 1, true);
		} catch (MqttException e) {
			/* Ignore */
		}
	...
	public static void publish(String name, Object val, String src, String dpt,
			String textual, long updateTime, long lastChange) {
		instance.doPublish(name, val, src, dpt, textual, updateTime,
				lastChange);
	}
